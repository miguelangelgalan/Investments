...
public class Damas implements HYIPInterface {

	private static final String NAME="Damas";
	private String baseUrl = "http://www.spalumi.com/";
	private String loginUrl = "/login.php?do=login";
	...
	private String visitUrl = "/f14/damas-del-placer-nueva-casa-relax-tu-escort-super-completas-170129/index100.html";
	private MechanizeAgent agent = null;
	private Logger log = Logger.getLogger("DAMAS");
		...
		try {
			...
			if (forms.size() != 0) {
				...
				for (Form f : forms) { //Nos quedamos con el de withdraw, que es el que no tiene nombre, y campo oculto a=withdraw
					if (f.getAttribute("action") != null) {
						if (f.getAttribute("action").startsWith("/login")) {
							loginForm = f;
						  break;
						}
					}						
			...
			if (loginForm != null) {
				FormElement username = loginForm.get("vb_login_username");
				username.setValue(user);
				FormElement password = loginForm.get("vb_login_password");
	...
	public double visitaURL() {
		
		
		String sAmount="0.0";
		int a=0;
		//if (isLogged()) {
			HtmlDocument pg = getAgent().get(baseUrl + visitUrl);
			if (check_OK(pg)) {
...
//		}
	...
	public boolean pillado() {
				

		int a=0;
			HtmlDocument pg = getAgent().get(baseUrl + visitUrl);
			if (check_OK(pg)) {
				String res = pg.asString();
				if (res.contains("pillado") || res.contains("PILLADO")) {
					return true;					
				} else {
					return false;
				}
