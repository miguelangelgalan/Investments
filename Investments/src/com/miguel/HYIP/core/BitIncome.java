...
public class BitIncome implements HYIPInterface {

	private static final String NAME="BitIncome";
	private String baseUrl = "https://bitincome.biz";
	private String loginUrl = "/?a=login";
	...
	private Logger log = Logger.getLogger("BITINCOME");
		...
		if (isLogged()) {
			...
			if (check_OK(pg)) {
				...
				for (HtmlElement e : divs) {
					...
					if (attr.equalsIgnoreCase("blue_background")) {
						// Buscamos td's
						List<HtmlElement> tds = (List<HtmlElement>) e.findAll("td");
						for (HtmlElement td : tds) {

							String padre = td.getParent().getChildren().get(1).getValue();
							if (padre.startsWith("Account Balance:")) {
								HtmlNode b = td.getParent().getChildren().get(3).find("b");
								sAmount = b.getValue().replace('$', ' ');
					...
					if (attr.equalsIgnoreCase("blue_background")) {
						...
						for (HtmlElement td : tds) {
							...
							if (padre.startsWith("Active Deposit:")) {
								HtmlNode b = td.getParent().getChildren().get(3).find("b");
								activeDepositAmount = Double.parseDouble(b.getValue().replace('$', ' '));
								System.out.println("ActiveDeposit: " + activeDepositAmount);												
							} else if (padre.startsWith("Last Deposit:")) {
								HtmlNode b = td.getParent().getChildren().get(3).find("b");
								lastInvestmentAmount = Double.parseDouble(b.getValue().replace('$', ' '));
								System.out.println("lastInvestmentAmount: " + lastInvestmentAmount);												
