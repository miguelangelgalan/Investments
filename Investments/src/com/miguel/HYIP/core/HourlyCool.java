...
public class HourlyCool implements HYIPInterface {

	private static final String NAME="HourlyCool";
	private String baseUrl = "https://hourlycool.com";
	...
	private Logger log = Logger.getLogger("HOURLYCOOL");
		...
		if (isLogged()) {
			...
			if (check_OK(pg)) {
				...
				for (HtmlElement e : tds) {
					String attr = e.getAttribute("bgcolor");
					if (attr.equalsIgnoreCase("#74A2D1")) {
						
						String padre = e.getParent().getValue();
						if (padre.startsWith("Account Balance:")) {
							
							//List <HtmlElements> bs = e.getParent().getChildren().get(3).find("b").getValue();
							sAmount = e.getParent().getChildren().get(3).find("b").getValue();
				...
				for (HtmlElement e : tds) {
					...
					if (attr.equalsIgnoreCase("#74A2D1")) {
						...
						if (padre.startsWith("Active Deposit:")) {
							...
							//List <HtmlElements> bs = e.getParent().getChildren().get(3).find("b").getValue();
							activeDepositAmount = e.getParent().getChildren().get(3).find("b").getValue();
